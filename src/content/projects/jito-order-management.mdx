...
- The project initially shipped as a Docker Compose stack, but the current design uses a self-hosted GitHub Actions runner with PM2 — the workflow writes runtime environment config, syncs main, rebuilds both apps, reloads them under the process manager, and verifies a backend health endpoint before marking the rollout successful.
...
- **CI/CD migration aligned with team maintenance model**: The initial Docker Compose deploy path was rebuilt around a self-hosted runner and a process-managed runtime (PM2). The change served two goals — making automated deploys actually ship inside a locked-down internal network, and right-sizing the tooling surface to a team where long-term maintainers aren't full-time developers. Trade-off: less build-time flexibility in exchange for operational reliability and onboarding simplicity.
...
- CI/CD with a self-hosted GitHub Actions runner and PM2-based deploys, landing a fully automated, health-checked rollout pipeline.
- Scheduled backups covering database dumps, uploaded images, and environment snapshots with retention.
